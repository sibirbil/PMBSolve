...
function pmbsolve(fun, x0, pars=pars)
...
#      OUTPUT.niter: Number of iterations
  ...
  while(true) # Outer Iterations ---->>
    ...
    if (output.fcalls >= pars.maxfcalls)
      output.exit = -2
      if (pars.message)
        @printf("Maximum number of function calls (maxfcalls) is reached\n")
      end
      break
    end
    if ((time() - tstart) >= pars.maxtime)
      output.exit = -3
      if (pars.message)
